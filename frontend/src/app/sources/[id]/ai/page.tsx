import { notFound } from "next/navigation";
import { createSupabaseServerClient } from "@/lib/supabase/server";
import { AIPage } from "@/components/ai/ai-page";
import { PageHeader } from "@/components/ui/page-header";
import type { Dataset } from "@/types";

export const dynamic = "force-dynamic";

export default async function AIRoute({
  params,
}: {
  params: Promise<{ id: string }>;
}) {
  const { id } = await params;
  const supabase = await createSupabaseServerClient();
  const { data, error } = await supabase
    .from("datasets")
    .select(
      "id,user_id,name,source_type,storage_path,row_count,column_count,status,created_at",
    )
    .eq("id", id)
    .maybeSingle();
  if (error || !data) return notFound();
  const ds = data as Dataset;

  return (
    <div className="space-y-6">
      <PageHeader
        title={`AI insights · ${ds.name}`}
        back={{ label: ds.name, href: `/sources/${ds.id}` }}
        description="Summary, data story, auto-insights, and a chat that plans and runs analyses against your data — without ever sending raw rows to the model."
      />
      <AIPage datasetId={ds.id} />
    </div>
  );
}
